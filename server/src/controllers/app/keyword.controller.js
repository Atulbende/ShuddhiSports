...
const getGridSizes = _async(async (req, res) => {
        // console.log('keywords:',req.cookies);
        // const Token= req.cookies?._sessionId;
        console.log('Token:');
        try {
                const sizes = await executeQuery('select * from size');
                return res.send(new APIResponse(200, "done", { data: sizes }));
        } catch (error) {
                return res.send(new ApiError(200, 'Somthing went Wrong'))
        }

})
const sizeSave = _async(async (req, res) => {
        try {
                const Pid = req.body.Pid;
                const Size = req.body.Size;
                const result = await executeQuery('call SizeSave(?,?,@Per_Result)', [Size, Pid]);
                const _msg = com_message(result[0].Per_Result);
                return res.send(new APIResponse(200, "done", { result: _msg }));
        } catch (error) {
...
const openSize = _async(async (req, res) => {

        const _id = req?.body?.Pid;
        if (_id === -1) {
                const result = await executeQuery(`SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=? AND TABLE_SCHEMA = ?`, ['size', process.env.DATABASE_NAME]);
                res.result = getNewObj(result);
        } else {
                const result = await executeQuery(`SELECT Pid,Size FROM size WHERE Pid=?;`, [_id]);
                console.log('result:', result, _id)
                res.result = result;
        }

        const requiredfields = await executeQuery(`select FieldsArray from requiredfields where TableName=?;`, ['size']);
        res.required = JSON.parse(requiredfields[0].FieldsArray.replace(/'/g, '"'));
        return res.send(new APIResponse(200, "loaded", { result: res.result, required: res.required }));
});

export {
        categorySave, getGridCategorys, openCategory,
        getGridBrands, brandSave, openBrand,
        sizeSave, getGridSizes, openSize
};
